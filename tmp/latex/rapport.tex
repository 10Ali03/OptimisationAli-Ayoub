...
\begin{itemize}
    \item \textbf{overview} (défaut) : affiche le résumé des instances, la recherche du nombre minimal de véhicules en CVRP et VRPTW, la génération de solutions aléatoires, et une première comparaison des métaheuristiques ;
...
\end{itemize}
Ce module inclut également la fonction \texttt{find\_minimum\_vehicles}, qui incrémente \(k\) depuis la borne inférieure capacitaire jusqu'à trouver un nombre de véhicules faisable.
...
Implémente le recuit simulé. À chaque itération, un voisin faisable est tiré aléatoirement via l'un des trois opérateurs. Il est accepté systématiquement s'il améliore la solution courante, ou avec probabilité $\exp(-\Delta/T)$ sinon — où $T$ décroît géométriquement à chaque itération selon le facteur de refroidissement. La meilleure solution rencontrée est mémorisée tout au long de la recherche, indépendamment de la solution courante.
...
Cette campagne longue confirme et clarifie plusieurs tendances déjà visibles en mode rapide. Les résultats ci-dessus correspondent à des moyennes sur cinq répétitions ; le temps tabou de \texttt{data1101.vrp} est reporté avec un astérisque car le run~3 a enregistré un temps de \(5899\) s dû à une suspension système (la valeur \(214\) s correspond à la moyenne des quatre autres runs).

\begin{itemize}
    \item Le recuit simulé améliore systématiquement les solutions initiales sur l'ensemble du corpus.
    \item La recherche tabou améliore elle aussi systématiquement les solutions et devient meilleure que le recuit simulé sur la totalité des dix instances VRPTW.
    \item Sur \texttt{data101.vrp} (l'instance la plus contrainte, \(k=21\)), la tabou l'emporte nettement : \(1683.01\) contre \(1706.28\) pour le recuit simulé.
    \item Les avantages sont particulièrement nets sur \texttt{data1201.vrp} (\(1445.97\) contre \(1544.00\)), \texttt{data201.vrp} (\(1268.26\) contre \(1378.63\)) et \texttt{data202.vrp} (\(1156.67\) contre \(1331.76\)).
    \item Le coût en temps de la recherche tabou augmente sensiblement sur les cas difficiles (plusieurs minutes par instance pour \texttt{data101.vrp} et \texttt{data1101.vrp}), mais reste raisonnable sur la majorité du corpus.
\end{itemize}

Nous observons que la recherche tabou fournit de meilleures solutions que le recuit simulé sur les dix instances VRPTW. Ce comportement s'explique par sa capacité à éviter plus efficacement les cycles tout en maintenant une exploration dirigée de l'espace faisable malgré les blocages locaux, et il est renforcé ici par les opérateurs Or-opt(2) et 2-opt$^*$ qui donnent à la tabou un espace de déplacement plus riche. Cette campagne longue confirme donc notre lecture finale du projet : dans le cas VRPTW, la recherche tabou correctement paramétrée est systématiquement meilleure que le recuit simulé, au prix d'un surcoût en temps notable sur les cas les plus contraints.
...
\midrule
CVRP  & data101.vrp & recuit simulé   & 1248.02 & 1328.85 & 29.7 \\
CVRP  & data101.vrp & recherche tabou & 1284.23 & 1453.67 & 57.8 \\
...
Instance & \(k\) & Init. moy. & RS moy. & Tabou moy. & Temps RS (s) & Temps tabou (s) \\
\midrule
data101.vrp  & 8 & 3655.35 & 1305.33 & 1367.68 & 2.55  & 10.45 \\
data102.vrp  & 8 & 3537.80 & 1293.09 & 1365.31 & 2.58  & 11.09 \\
data1101.vrp & 9 & 4718.00 & 1698.95 & 1747.80 & 4.01  & 16.24 \\
data1102.vrp & 9 & 4675.37 & 1630.59 & 1723.34 & 3.90  & 16.35 \\
data111.vrp  & 8 & 3519.79 & 1303.22 & 1350.86 & 2.23  &  8.94 \\
data112.vrp  & 8 & 3542.86 & 1278.01 & 1358.68 & 2.23  &  9.05 \\
data1201.vrp & 2 & 4613.08 & 1348.31 & 1327.67 & 1.34  &  5.27 \\
data1202.vrp & 2 & 4366.31 & 1310.44 & 1343.73 & 1.31  &  5.51 \\
data201.vrp  & 2 & 3580.75 & 1136.44 & 1177.15 & 1.04  &  4.51 \\
data202.vrp  & 2 & 3379.91 & 1116.25 & 1149.75 & 1.22  &  4.72 \\
\bottomrule
\end{tabular}
\end{center}
...
Trois résultats ressortent nettement. D'abord, sur le corpus CVRP étudié, la borne inférieure capacitaire est atteinte pour toutes les instances et coïncide donc avec le nombre minimal de véhicules observé. Ensuite, l'introduction des fenêtres de temps augmente fortement le nombre de véhicules nécessaires et resserre l'espace des solutions faisables. Enfin, les performances relatives des métaheuristiques dépendent de cette structure : le recuit simulé constitue le meilleur compromis en CVRP (9 instances sur 10), alors que la recherche tabou devient systématiquement meilleure en VRPTW sur la totalité du corpus dans la campagne longue finale.

\subsection{Comment nous cherchons l'optimum}

Notre stratégie de résolution repose sur une décomposition en trois couches.

\textbf{Couche 1 — Construction.} Nous déterminons d'abord le nombre minimal de véhicules $k$ par une heuristique gloutonne géographique, en incrémentant depuis la borne inférieure capacitaire $LB$ jusqu'à obtenir une solution faisable. En VRPTW, une construction déterministe par insertion prend le relais lorsque la construction aléatoire échoue. Cette solution initiale fournit un point de départ réaliste mais sous-optimal.
...
\textbf{Mémoire tabou par signature globale.} Nous encodons chaque solution par sa signature complète (tuple trié des routes), qui est une représentation grossière : deux solutions très différentes topologiquement peuvent produire la même signature si elles visitent les mêmes clients dans des ordres différents. Une mémoire tabou basée sur les mouvements eux-mêmes (paire de clients concernés, type d'opérateur) serait plus précise et permettrait un tenure plus long sans sur-interdire de bons candidats.

...
\midrule
data101.vrp  & 21 & 2127.09 & 1906.31 & 2078.33 & 3.51 & 13.47 \\
data102.vrp  & 18 & 1794.68 & 1703.70 & 1753.13 & 0.93 & 12.37 \\
data1101.vrp & 18 & 2184.86 & 2073.22 & 2105.62 & 1.60 & 12.29 \\
data1102.vrp & 16 & 2035.58 & 1979.05 & 1985.54 & 1.03 & 11.75 \\
data111.vrp  & 14 & 1601.37 & 1601.37 & 1576.06 & 0.92 & 11.25 \\
data112.vrp  & 12 & 1434.70 & 1392.79 & 1395.91 & 0.73 & 10.72 \\
data1201.vrp & 5  & 1974.63 & 1973.68 & 1945.36 & 0.96 & 9.38 \\
data1202.vrp & 4  & 1948.06 & 1918.80 & 1870.66 & 0.62 & 9.30 \\
data201.vrp  & 5  & 1750.26 & 1748.15 & 1713.03 & 0.64 & 9.32 \\
data202.vrp  & 5  & 3113.87 & 2134.61 & 2959.75 & 0.33 & 9.26 \\
\bottomrule
\end{tabular}
\end{center}
...
Ces résultats sont encore préliminaires, mais ils montrent que la hiérarchie observée dans la version sans fenêtres de temps n'est pas automatiquement identique dans le cas VRPTW. La présence des contraintes temporelles modifie non seulement la faisabilité des tournées, mais aussi le comportement comparé des métaheuristiques.
...
\midrule
data101.vrp  & 21 & 2127.09 & 1756.06 & 2042.71 & 18.55 & 26.33 \\
data102.vrp  & 18 & 1794.68 & 1683.80 & 1695.11 & 4.38 & 25.26 \\
data1101.vrp & 18 & 2184.86 & 1851.01 & 2036.87 & 5.46 & 15.54 \\
data1102.vrp & 16 & 2035.58 & 1765.05 & 1955.24 & 2.73 & 13.82 \\
data111.vrp  & 14 & 1601.37 & 1426.25 & 1557.55 & 1.80 & 13.03 \\
data112.vrp  & 12 & 1434.70 & 1256.10 & 1367.21 & 2.33 & 12.67 \\
data1201.vrp & 5  & 1974.63 & 1833.03 & 1864.35 & 1.79 & 11.14 \\
data1202.vrp & 4  & 1948.06 & 1746.99 & 1861.51 & 1.52 & 10.78 \\
data201.vrp  & 5  & 2263.65 & 1656.57 & 2074.35 & 1.47 & 10.97 \\
data202.vrp  & 5  & 2997.76 & 1772.69 & 2797.17 & 0.78 & 10.86 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item La recherche tabou améliore elle aussi les solutions, mais reste globalement moins performante que le recuit simulé dans notre implémentation actuelle.
    \item L'écart de qualité devient parfois très marqué, notamment sur \texttt{data101.vrp}, \texttt{data201.vrp} et \texttt{data202.vrp}.
    \item Le coût en temps du recuit simulé dépend fortement de l'instance, mais reste très inférieur ou comparable à celui de la recherche tabou sur la majorité des cas.
    \item La recherche tabou demeure plus stable en temps, souvent entre \(10\) et \(26\) secondes, au prix d'un gain plus limité.
\end{itemize}

Cette campagne longue renforce donc la conclusion générale du projet : même dans le cas VRPTW, le recuit simulé fournit actuellement le meilleur compromis entre qualité de solution et coût de calcul sur les instances étudiées.
...
Le temps de calcul moyen mesuré dans ce mode rapide est d'environ \(0.10\) seconde pour le recuit simulé contre environ \(12.02\) secondes pour la recherche tabou. Cette différence confirme que, dans notre implémentation actuelle, la recherche tabou est nettement plus coûteuse et devra être utilisée avec précaution lors des campagnes expérimentales longues.
...
Un premier test en mode \texttt{long} a ensuite été lancé sur l'instance \texttt{data101.vrp}, avec deux répétitions. La distance moyenne initiale observée est \(3719.84\). Dans ce cadre plus exigeant, le recuit simulé obtient une distance moyenne de \(1800.80\), alors que la recherche tabou atteint une distance moyenne de \(3424.76\).

Les temps moyens associés à cette expérience sont d'environ \(0.36\) seconde pour le recuit simulé et \(19.96\) secondes pour la recherche tabou.
...
\begin{itemize}
    ...
    \item la recherche tabou reste beaucoup plus coûteuse dans l'état actuel de notre implémentation, tout en apportant ici une amélioration plus limitée.
\end{itemize}
...
Nous avons ensuite appliqué le même protocole \texttt{long} à l'instance \texttt{data102.vrp}. La distance moyenne initiale obtenue est \(3609.70\). Le recuit simulé atteint alors une distance moyenne de \(1764.93\), tandis que la recherche tabou obtient une distance moyenne de \(3372.76\).

Les temps moyens observés sont du même ordre que pour \texttt{data101.vrp} : environ \(0.37\) seconde pour le recuit simulé contre environ \(20.71\) secondes pour la recherche tabou.
...
Pour \texttt{data1101.vrp}, la distance moyenne initiale observée est \(4717.62\). Le recuit simulé obtient une distance moyenne de \(2271.42\), tandis que la recherche tabou atteint \(4448.35\).

Pour \texttt{data1102.vrp}, la distance moyenne initiale est \(4683.40\). Le recuit simulé descend à \(2368.66\) en moyenne, alors que la recherche tabou obtient \(4311.87\).

Les temps moyens restent dans le même ordre de grandeur que précédemment : environ \(0.48\) seconde pour le recuit simulé et environ \(20.3\) secondes pour la recherche tabou.
...
Le même protocole \texttt{long} a ensuite été appliqué aux instances \texttt{data111.vrp} et \texttt{data112.vrp}. Les résultats observés sont identiques à ceux obtenus respectivement sur \texttt{data101.vrp} et \texttt{data102.vrp}, avec les mêmes graines expérimentales.

Autrement dit :
\begin{itemize}
    \item \texttt{data111.vrp} reproduit les valeurs observées sur \texttt{data101.vrp} ;
    \item \texttt{data112.vrp} reproduit les valeurs observées sur \texttt{data102.vrp}.
\end{itemize}

Cette répétition n'est pas anodine. Elle suggère que ces paires d'instances possèdent une structure très proche, voire équivalente pour les heuristiques utilisées dans notre étude. Du point de vue expérimental, cela renforce la cohérence des résultats déjà obtenus et nous donne davantage de confiance dans la stabilité des tendances observées.
...
Pour \texttt{data1201.vrp}, la distance moyenne initiale observée est \(4507.39\). Le recuit simulé atteint une distance moyenne de \(1898.40\), tandis que la recherche tabou obtient \(4191.42\).

Pour \texttt{data1202.vrp}, la distance moyenne initiale vaut \(4693.59\). Le recuit simulé descend à \(2051.32\) en moyenne, alors que la recherche tabou atteint \(4216.21\).

Le point notable ici est que le recuit simulé devient encore plus rapide, avec un temps moyen d'environ \(0.26\) seconde sur ces instances à deux tournées, tandis que la recherche tabou reste autour de \(19\) à \(20\) secondes.
...
Pour \texttt{data201.vrp}, la distance moyenne initiale observée est \(3410.18\). Le recuit simulé obtient une distance moyenne de \(1547.01\), tandis que la recherche tabou atteint \(3111.51\).

Pour \texttt{data202.vrp}, la distance moyenne initiale vaut \(3436.16\). Le recuit simulé descend à \(1660.53\) en moyenne, alors que la recherche tabou obtient \(3129.50\).

Comme pour \texttt{data1201.vrp} et \texttt{data1202.vrp}, le recuit simulé reste ici très rapide, autour de \(0.26\) seconde, tandis que la recherche tabou demeure proche de \(19\) secondes.
...
\begin{itemize}
    ...
    \item la recherche tabou améliore également les solutions, mais de manière beaucoup plus limitée dans notre implémentation actuelle ;
    \item le coût de calcul du recuit simulé reste faible sur toutes les instances testées ;
    \item le coût de calcul de la recherche tabou reste élevé et relativement stable, généralement autour de \(20\) secondes par bloc expérimental.
\end{itemize}
...
Instance & \(k\) & Init. moy. & RS moy. & Tabou moy. & Temps RS & Temps tabou \\
\midrule
data101.vrp  & 8 & 3719.84 & 1800.80 & 3424.76 & 0.37 & 20.25 \\
data102.vrp  & 8 & 3609.70 & 1764.93 & 3372.76 & 0.37 & 20.71 \\
data1101.vrp & 9 & 4717.62 & 2271.42 & 4448.35 & 0.47 & 20.27 \\
data1102.vrp & 9 & 4683.40 & 2368.66 & 4311.87 & 0.48 & 20.35 \\
data111.vrp  & 8 & 3719.84 & 1800.80 & 3424.76 & 0.37 & 20.69 \\
data112.vrp  & 8 & 3609.70 & 1764.93 & 3372.76 & 0.36 & 20.08 \\
data1201.vrp & 2 & 4507.39 & 1898.40 & 4191.42 & 0.26 & 19.91 \\
data1202.vrp & 2 & 4693.59 & 2051.32 & 4216.21 & 0.26 & 19.34 \\
data201.vrp  & 2 & 3410.18 & 1547.01 & 3111.51 & 0.26 & 19.22 \\
data202.vrp  & 2 & 3436.16 & 1660.53 & 3129.50 & 0.26 & 19.16 \\
\bottomrule
\end{tabular}
\end{center}

Cette première campagne ne constitue pas encore l'étude expérimentale finale du projet, mais elle fournit déjà un signal clair : avec les choix d'implémentation et les réglages actuels, le recuit simulé apparaît comme la méthode la plus convaincante parmi les deux métaheuristiques testées.
...
\begin{itemize}
    \item fiabilisation du générateur aléatoire de solutions initiales dans le cas avec fenêtres de temps ;
    \item éventuel raffinement de la recherche tabou, qui reste en retrait sur la majorité des campagnes menées ;
    \item nettoyage final du projet avant rendu.
\end{itemize}
...
\begin{itemize}
    ...
    \item implémenté la lecture des instances au format \texttt{.vrp} ;
    \item mis en place un module d'évaluation unifié pour les distances, les charges et les contraintes temporelles ;
    \item déterminé expérimentalement le nombre minimal de véhicules dans la version sans fenêtres de temps sur les instances fournies ;
    \item intégré une première construction faisable pour la version avec fenêtres de temps ;
    \item implémenté un générateur de solutions initiales pour la version sans fenêtres de temps ;
    \item développé deux métaheuristiques, le recuit simulé et la recherche tabou ;
    \item mené une première campagne expérimentale complète sans fenêtres de temps ;
    \item mené une campagne expérimentale complète en mode rapide puis en mode long sur la version avec fenêtres de temps ;
    \item comparé explicitement les deux approches en temps, en qualité et en volume de voisins traités ;
    \item réalisé une première étude bonus par résolution exacte sur des sous-instances croissantes.
\end{itemize}

Les résultats obtenus mettent en évidence trois enseignements principaux. D'une part, les fenêtres de temps modifient profondément la structure du problème et augmentent fortement le nombre de véhicules nécessaires. D'autre part, dans notre implémentation actuelle, le recuit simulé fournit globalement le meilleur compromis entre qualité de solution et temps de calcul, aussi bien dans la version sans fenêtres de temps que dans les premiers essais VRPTW. Enfin, l'étude exacte avec OR-Tools montre qu'une résolution par programmation linéaire devient déjà sensiblement plus délicate autour de \(30\) clients dans notre cadre expérimental.

Le projet dispose donc désormais d'une base cohérente, à la fois du point de vue algorithmique et du point de vue expérimental. Les améliorations restantes concernent surtout l'approfondissement de la version VRPTW et la consolidation finale des résultats.

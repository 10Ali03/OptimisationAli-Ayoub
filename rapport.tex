...
\midrule
Itérations recuit simulé & 300 & 1200 \\
Température initiale & 100.0 & 120.0 \\
Facteur de refroidissement & 0.98 & 0.995 \\
Tentatives de voisinage par itération (RS) & 50 & 100 \\
Itérations recherche tabou & 5 & 20 \\
Tenure tabou & 5 & 12 \\
Voisins max évalués par itération & 40 & 100 \\
Répétitions par instance & 1 & 2 \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
CVRP & data101.vrp & recuit simulé & 1769.01 & 1859.75 & 45.37 \\
CVRP & data101.vrp & recherche tabou & 2398.14 & 2441.57 & 21.72 \\
VRPTW & data101.vrp & recuit simulé & 1754.56 & 1757.56 & 1.50 \\
VRPTW & data101.vrp & recherche tabou & 1851.41 & 1854.09 & 1.34 \\
VRPTW & data202.vrp & recuit simulé & 1479.07 & 1534.13 & 27.53 \\
VRPTW & data202.vrp & recherche tabou & 1255.46 & 1310.01 & 27.27 \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{data101.vrp} constitue d'abord le cas VRPTW le plus contraint de notre corpus, avec \(21\) véhicules nécessaires contre \(8\) en CVRP. Comme montré par les tableaux et la courbe de convergence dédiée, ce resserrement extrême de l'espace faisable favorise encore le recuit simulé en qualité finale (\(1756.06\) contre \(1852.75\) pour la tabou), mais au prix d'un temps déjà élevé pour les deux méthodes et surtout pour la tabou. Cette instance joue donc le rôle de cas limite, utile pour comprendre le coût réel des contraintes temporelles les plus sévères.

\texttt{data1201.vrp} est ensuite un cas charnière, car il illustre très clairement le changement de régime entre CVRP et VRPTW. Sans fenêtres de temps, \(2\) véhicules suffisent et le recuit simulé domine nettement (\(2001.83\) contre \(2905.82\)). Avec fenêtres de temps, l'instance passe à \(5\) véhicules et la recherche tabou devient meilleure (\(1752.22\) contre \(1840.63\)). La visualisation des tournées montre concrètement cette fragmentation supplémentaire et rend visible l'effet structurel des contraintes temporelles.

Enfin, \texttt{data202.vrp} constitue un cas atypique très instructif en VRPTW. Sur cette instance, le recuit simulé ne parvient pratiquement pas à améliorer la solution initiale moyenne (\(1506.60\) avant comme après optimisation), alors que la recherche tabou descend jusqu'à \(1282.73\). Ce comportement suggère que certains bassins locaux deviennent particulièrement difficiles à quitter lorsque l'espace faisable est très resserré, et qu'une exploration plus structurée avec mémoire peut alors devenir décisive.
...
Ces conclusions reposent sur l'ensemble du corpus final et non sur quelques exemples isolés ; les études de cas détaillées servent seulement à interpréter les comportements les plus marquants. Les principales limites concernent la robustesse encore partielle du générateur aléatoire en VRPTW et le coût plus élevé de la recherche tabou sur certains cas extrêmes. Des améliorations naturelles incluent l'ajout de voisinages plus complexes, de stratégies hybrides et d'approches exactes sur de petits sous-problèmes.

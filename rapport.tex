...
Nous avons décidé de traiter cette question en deux temps :
\begin{itemize}
...
Cette borne est nécessaire mais pas toujours suffisante, car elle ne tient compte que de la capacité totale et pas de la répartition effective des clients dans les tournées.

\subsection{Méthode envisagée}
Pour une instance donnée :
\begin{enumerate}[label=\arabic*.]
    \item calculer la borne inférieure \(LB\) ;
    \item essayer de construire des solutions faisables avec exactement \(LB\) véhicules ;
    \item si aucune solution faisable n'est trouvée après un certain nombre d'essais, tester avec \(LB+1\), puis \(LB+2\), etc. ;
    \item retenir le premier nombre de véhicules pour lequel une solution faisable est obtenue.
\end{enumerate}

Cette stratégie sera d'abord appliquée sans fenêtres de temps, puis avec fenêtres de temps.
